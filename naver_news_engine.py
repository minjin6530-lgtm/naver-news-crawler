...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[작성 규칙 — 각색 금지, 원문 핵심 내용 그대로]
- 문장 한 개로 작성
- 반드시 해당 기사 자신의 title_korean과 description_raw에 명시적으로 쓰인
  내용만 사용할 것. 각색·추론·짐작으로 내용을 보태거나 그럴듯하게 다듬지 않는다.
- 같은 사건을 다룬 다른 배치/다른 기사의 내용을 가져와 섞지 말 것. 각 기사는
  그 기사 자신의 title_korean/description_raw만 근거로 요약한다.
- 배경지식(예: 특정 AI 모델이 어느 회사 제품인지 등)으로 원문에 없는 내용을
  덧붙이지 말 것.
- description_raw는 "..."로 끊긴 발췌문이다. 잘린 부분 뒤에 어떤 내용이
  이어질지 추측하지 말고, 확실히 나온 문장까지만 사용할 것.
- 수치/데이터가 있으면 원문 표현 그대로(단위·괄호 안 수치 포함) 가져올 것.
- 플레이스 관점에서 중요한 키워드(예약, 수수료, 수요 변화, 플랫폼 전략 등)가
  원문에 있으면 반드시 포함
- '플레이스', '우리 사업' 표현 사용 금지
